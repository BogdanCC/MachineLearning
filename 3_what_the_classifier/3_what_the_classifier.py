...
# The purpose of today is to think of the algorithm used by the model(classifier) as a function

# Import and load toy iris data set
from sklearn import datasets as ds
iris_data = ds.load_iris()

# Now we will name the features and labels X and y accordingly, in order to think of this as a function
# f(x) = y ; where x = features and y = labels
X = iris_data.data # features
y = iris_data.target # labels

# Now we can split the data from the iris dataset in half, so we have our training data and our testing data
...
# Train classifiers with the training data
clfT.fit(X_train, y_train)
clfG.fit(X_train, y_train)
clfN.fit(X_train, y_train)

# Predict the label for these features (testing data)
